...
<p align="center">
  <img src="./public/logo.svg" alt="Caltrack" width="360">
</p>
...
Caltrack is a mobile-first Next.js PWA for private food logging. Scan a packaged item, fetch nutrition from Open Food Facts, confirm the label, and save it to a flat daily timeline. If there is no barcode, describe the food and optionally attach a photo to prefill the same review sheet with a Gemini estimate before saving.

The input model is intentionally small: barcode scan first, estimate or manual fallback only when a barcode is not available. No calorie goals, no meal buckets, no photo storage, no server management.
...
| Estimate fallback | Gemini Flash Lite |
| Runtime | Bun |
| Deploy | Vercel + Turso |
...
- Food photos are only transient estimate inputs and are not stored.
